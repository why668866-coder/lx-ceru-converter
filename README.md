...
- 基于 Node.js 的命令行工具，实现 **洛雪音乐（LX Music）** 与 **澜音音乐（CeruMusic）** 之间的歌单双向转换，完全兼容澜音官方的 `.cmpl`/`.cpl` 加密格式。
- **纯前端、零依赖**的 Web 工具，无需安装任何软件或 Node.js，即可在浏览器中完成 **洛雪音乐（LX Music）** 与 **澜音音乐（CeruMusic）** 之间的歌单格式互转。
- **新手使用ai编程制作，如有问题，望谅解。**
...
### 1. 方向切换
页面顶部有两个选项卡：
- **洛雪 → 澜音**：上传洛雪歌单，转换为澜音加密文件。
- **澜音 → 洛雪**：上传澜音加密文件，还原为洛雪 JSON。

### 2. 上传文件
- 点击上传区域或拖拽文件到虚线框内。
- 支持的文件类型：
  - 洛雪：`.json`、`.lxmc`（gzip 压缩的 JSON）
  - 澜音：`.cmpl`、`.cpl`

### 3. 预览歌单
上传成功后，会以分组形式展示每个歌单的名称、歌曲数量及完整列表，方便确认内容。

### 4. 转换选项（仅洛雪→澜音）
- **合并所有歌单**：勾选后将所有歌曲混入一个文件（丢失歌单边界）。
- **输出格式**：`.cmpl`（推荐，加密后 gzip 压缩）或 `.cpl`（仅加密文本，旧版兼容）。
...
| 模块 | 技术 | 说明 |
|------|------|------|
| 加密/解密 | **CryptoJS** | AES-256-CBC，OpenSSL 兼容模式，密钥固定为 `CeruMusic-PlaylistSecretKey` |
| 压缩/解压 | **浏览器原生 CompressionStream / DecompressionStream** | 用于处理 `.lxmc` 和 `.cmpl` 的 gzip 操作 |
| ZIP 打包 | **JSZip** | 多歌单导出时自动打包为 ZIP |
| 文件保存 | **FileSaver.js** | 跨浏览器的文件下载，降级使用 a 标签 |

- **完全前端运行**：所有计算均在本地完成，不上传任何数据，保护隐私。
- **无需安装**：一个 HTML 文件即开即用。
